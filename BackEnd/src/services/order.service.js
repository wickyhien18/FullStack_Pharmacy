...
// Tạo đơn hàng
export const createOrder = async (userId, { items, shippingAddress, paymentMethod, note }) => {
  if (!items?.length) throw { status: 400, message: 'Đơn hàng không có sản phẩm' };
  if (!shippingAddress) throw { status: 400, message: 'Vui lòng nhập địa chỉ giao hàng' };
