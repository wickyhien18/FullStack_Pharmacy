...
export default function MedicineFormModal({ medicineId, onClose }) {
  ...
  const [form, setForm] = useState({
    name: "", description: "", price: "", unit: "Hộp",
    categoryId: "", manufacturerId: "", status: "ACTIVE", stock: 0,
  });
  ...
  const { data: detail, isLoading: isLoadingDetail } = useQuery({
    ...
    queryFn:  () => api.get(`/admin/medicines/${medicineId}`).then((r) => r.data.data),
    enabled:  isEdit,
  });
  ...
  const { data: categories } = useQuery({
    queryKey: ["categories"],
    queryFn:  () => api.get("/categories").then((r) => r.data.data.items),
  });
  const { data: manufacturers } = useQuery({
    queryKey: ["manufacturers"],
    queryFn:  () => api.get("/manufacturers").then((r) => r.data.data.items),
  });
  ...
  if (isEdit && isLoadingDetail) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div className="bg-white rounded-2xl p-8 text-sm text-gray-500">Đang tải...</div>
      </div>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                name="manufacturerId" value={form.manufacturerId} onChange={handleChange}
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-blue-400 bg-white"
              >
          ...
          <div className="flex gap-3 pt-2 border-t border-gray-100">
            ...
            >
              {mutation.isPending ? "Đang xử lý..." : isEdit ? "Lưu thay đổi" : "Thêm sản phẩm"}
            </button>
          </div>
        </form>
      </div>
