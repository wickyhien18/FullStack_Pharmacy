...
import {
  Filter, SlidersHorizontal, ChevronDown, ChevronRight, X,
} from "lucide-react";
import { ProductCard } from "../components/ProductCard";
import { useProducts, useCategoriesWithCount } from "../../hooks/useProducts.js"; // ← đổi import
...
const FALLBACK_IMAGE = "https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?w=400&h=400&fit=crop&auto=format";
...
function ProductListPage() {
  ...
  const { data: categoriesData, isLoading: isLoadingCategories } = useCategoriesWithCount();
  ...
  const totalMedicineCount = liveCategories.reduce((sum, c) => sum + (Number(c.count) || 0), 0);
  ...
  const totalPages = productsData?.totalPages || Math.ceil(total / ITEMS_PER_PAGE);
          ...
          <div className="bg-white rounded-xl p-4 mb-4">
            ...
            <div className="space-y-1">
              ...
              >
                ...
                <span className={`text-xs ${!categorySlug ? "text-white/70" : "text-gray-400"}`}>
                  {totalMedicineCount}
                </span>
              </button>
              {liveCategories.map((cat) => (
                <button
                  ...
                  style={categorySlug === cat.id ? { backgroundColor: "#1250dc" } : {}}
                >
                  <span>{cat.icon} {cat.name}</span>
                  <span className={`text-xs ${categorySlug === cat.id ? "text-white/70" : "text-gray-400"}`}>
                    {cat.count || "0"}
                  </span>
                </button>
              ))}
            ...
            <div className="space-y-1">
              {priceRanges.map((range) => {
                ...
                return (
                  <button
                    key={range.label}
                    onClick={() => { setPriceRange(active ? null : range); setPage(1); }}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="bg-white rounded-xl px-4 py-3 mb-4 flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-2 text-sm text-gray-600">
              ...
              {priceRange && (
                <button
                  onClick={() => { setPriceRange(null); setPage(1); }}
          ...
          {showFilter && (
            <div className="md:hidden bg-white rounded-xl p-4 mb-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => { handleCategoryChange(""); setShowFilter(false); }}
                ...
                {liveCategories.map((cat) => (
                  <button
                    key={cat.id}
                    onClick={() => { handleCategoryChange(cat.id); setShowFilter(false); }}
