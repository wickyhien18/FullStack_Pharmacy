...
function Countdown({ endDate }) {
  ...
  useEffect(() => {
    const calc = () => {
      const diff = Math.max(
        0,
        Math.floor((new Date(endDate).getTime() - Date.now()) / 1e3),
      );
      setTimeLeft({
...
function PromotionsPage() {
  const saleProducts = products
    .filter((p) => p.discount && p.discount >= 18)
    .slice(0, 8);
  return (
    <div className="max-w-7xl mx-auto px-4 py-5">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg, #1250dc, #0a3a9e)" }}
      >
        <div className="px-8 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-white">
            ...
            >
              Khuyến Mãi Long Châu
            </h1>
            <p className="text-white/80 text-sm">
              Hàng ngàn sản phẩm giảm giá mỗi ngày. Đặt hàng ngay để không bỏ
              lỡ!
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
        {promotions.map((promo) => (
          <Link
            ...
            to={
              promo.category
                ? `/products?category=${promo.category}`
                : "/products"
            }
      ...
      <div className="bg-white rounded-2xl p-6 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            {
              code: "LONGCHAU10",
              desc: "Gi\u1EA3m 10% cho \u0111\u01A1n h\xE0ng b\u1EA5t k\u1EF3",
              min: "Kh\xF4ng gi\u1EDBi h\u1EA1n",
            },
            {
              code: "FREESHIP50",
              desc: "Mi\u1EC5n ph\xED v\u1EADn chuy\u1EC3n cho \u0111\u01A1n t\u1EEB 50k",
              min: "\u0110\u01A1n t\u1EEB 50.000\u0111",
            },
            {
              code: "NEWUSER30",
              desc: "Gi\u1EA3m 30% cho kh\xE1ch h\xE0ng m\u1EDBi",
              min: "L\u1EA7n \u0111\u1EA7u mua h\xE0ng",
            },
          ].map((coupon) => (
            <div
              key={coupon.code}
              className="border-2 border-dashed border-blue-200 rounded-xl p-4 relative hover:border-blue-400 transition-colors cursor-pointer"
            >
      ...
      <section className="bg-white rounded-2xl p-6">
        <div className="flex items-center justify-between mb-5">
          <h2
            ...
            style={{ fontSize: "1.1rem" }}
          >
            <Zap size={20} className="text-red-500" fill="currentColor" /> Sản
            phẩm đang giảm giá
          </h2>
          <Link
            to="/products"
