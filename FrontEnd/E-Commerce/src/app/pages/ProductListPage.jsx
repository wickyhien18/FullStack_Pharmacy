...
function ProductListPage() {
  ...
  const matchedCategory = categoriesData?.items?.find(c => c.slug === categorySlug);
  ...
  const filtered = useMemo(() => {
    ...
    if (searchParams.get("sale") === "flash") list = list.filter((p) => p.isFlashSale);
    if (searchParams.get("sort") === "bestseller") list = list.filter((p) => p.isBestSeller);
    if (searchParams.get("sort") === "featured") list = list.filter((p) => p.isFeatured);
  ...
  return <div className="max-w-7xl mx-auto px-4 py-5">
    ...
    /* Breadcrumb */
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 mb-4">
        ...
        <span className="text-gray-800">{activeCategory?.name || "T\u1EA5t c\u1EA3 s\u1EA3n ph\u1EA9m"}</span>
      </nav>
  ...
  >
    ...
    style={category === cat.id ? { backgroundColor: "#1250dc" } : {}}
  >
                  <span>{cat.icon} {cat.name}</span>
                  <span className={`text-xs ${category === cat.id ? "text-white/70" : "text-gray-400"}`}>{cat.count.toLocaleString()}</span>
                </button>)}
            </div>
          </div>
  ...
  >
    ...
    /* Mobile filter */
          ...
          {showFilter && <div className="md:hidden bg-white rounded-xl p-4 mb-4">
    ...
    setShowFilter(false);
  }} className={`text-sm px-3 py-2 rounded-lg text-left ${!category ? "text-white" : "bg-gray-50 text-gray-700"}`} style={!category ? { backgroundColor: "#1250dc" } : {}}>
                  Tất cả
                </button>
                {liveCategories.map((cat) => <button key={cat.id} onClick={() => {
    handleCategoryChange(cat.id);
    setShowFilter(false);
  }} className={`text-sm px-3 py-2 rounded-lg text-left ${category === cat.id ? "text-white" : "bg-gray-50 text-gray-700"}`} style={category === cat.id ? { backgroundColor: "#1250dc" } : {}}>
                    {cat.icon} {cat.name}
                  </button>)}
              </div>
            </div>}
    ...
    /* Products grid */
          ...
          {paginated.length > 0 ? <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {paginated.map((p) => <ProductCard key={p.id} product={p} />)}
            </div> : <div className="bg-white rounded-xl p-16 text-center text-gray-500">
              <div className="text-4xl mb-3">🔍</div>
              <div className="font-medium text-gray-700 mb-1">Không tìm thấy sản phẩm</div>
    ...
    /* Pagination */
          ...
          {totalPages > 1 && <div className="flex justify-center items-center gap-2 mt-8">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-4 py-2 text-sm rounded-lg border border-gray-200 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed">
                Trước
              </button>
  ...
  >
              ...
              <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-4 py-2 text-sm rounded-lg border border-gray-200 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed">
                Sau
              </button>
            </div>}
        </div>
