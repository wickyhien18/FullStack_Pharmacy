...
const statusMap = {
  PENDING: { label: "Chờ xác nhận", style: "bg-yellow-50 text-yellow-700 border-yellow-200" },
  CONFIRMED: { label: "Đã xác nhận", style: "bg-blue-50 text-blue-700 border-blue-200" },
  SHIPPING: { label: "Đang giao hàng", style: "bg-indigo-50 text-indigo-700 border-indigo-200" },
  DELIVERED: { label: "Đã giao hàng", style: "bg-green-50 text-green-700 border-green-200" },
...
function AccountPage() {
  ...
  const handleRegisterSubmit = (e) => {
    e.preventDefault();
    if (!regForm.fullName || !regForm.userName || !regForm.email || !regForm.phone || !regForm.password) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="space-y-3">
            <div className="bg-white rounded-2xl p-5 text-center">
              ...
              <div className="text-sm text-gray-500">{user.phone || "@" + user.userName}</div>
            ...
            <div className="bg-white rounded-2xl overflow-hidden">
              {[
                { id: "profile", icon: <User size={16} />, label: "Thông tin tài khoản" },
                { id: "orders", icon: <ShoppingBag size={16} />, label: "Đơn hàng của tôi" },
              ].map((item) => (
                <button
                  key={item.id}
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 text-sm transition-colors border-b border-gray-50 last:border-0 text-left ${
                    activeSubTab === item.id ? "bg-blue-50 text-blue-700 font-semibold" : "text-gray-700 hover:bg-gray-50"
                  }`}
                >
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input disabled value={user.fullName || ""} className="w-full border border-gray-150 bg-gray-50 rounded-xl px-3 py-2.5 text-sm focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Tên đăng nhập</label>
                    <input disabled value={user.userName || ""} className="w-full border border-gray-150 bg-gray-50 rounded-xl px-3 py-2.5 text-sm focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Email</label>
                    <input disabled value={user.email || ""} className="w-full border border-gray-150 bg-gray-50 rounded-xl px-3 py-2.5 text-sm focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Số điện thoại</label>
                    <input disabled value={user.phone || ""} className="w-full border border-gray-150 bg-gray-50 rounded-xl px-3 py-2.5 text-sm focus:outline-none" />
                  </div>
                </div>
              </>
            ) : (
              <>
                <h2 className="font-semibold text-gray-800 mb-5" style={{ fontSize: "1rem" }}>Đơn hàng của tôi</h2>
                {isLoadingOrders ? (
                  ...
                  <div className="space-y-4">
                    {ordersList.map((order) => (
                      <div key={order.orderId} className="border border-gray-100 rounded-xl p-4 hover:shadow-sm transition-shadow">
                        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-50 pb-2 mb-3">
                          <div>
                            <span className="font-mono text-xs text-gray-500 font-semibold">{order.orderCode}</span>
                            <span className="text-xs text-gray-400 ml-3">{new Date(order.createdAt).toLocaleDateString("vi-VN")}</span>
                          </div>
                          <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${statusMap[order.orderStatus]?.style || "bg-gray-100 text-gray-600"}`}>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start max-w-4xl mx-auto">
        ...
        {/* Benefits panel */}
        <div className="space-y-4">
          <div className="rounded-2xl p-6 text-white shadow-sm animate-fade-in" style={{ backgroundColor: "#1250dc" }}>
            <h3 className="font-bold mb-3" style={{ fontSize: "1.1rem" }}>Lợi ích thành viên Long Châu</h3>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            ...
            <a href="tel:18006928" className="block text-center py-2.5 rounded-xl font-semibold text-sm text-white hover:opacity-90 transition-opacity" style={{ backgroundColor: "#1250dc" }}>
              📞 1800 6928 (Miễn phí)
            </a>
          </div>
        </div>
