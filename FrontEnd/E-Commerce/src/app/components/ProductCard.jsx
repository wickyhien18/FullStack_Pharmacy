...
function ProductCard({ product, showDiscount = true }) {
  ...
  return <Link to={`/products/${product.id}`} className="group bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg hover:border-blue-200 transition-all duration-200 flex flex-col">
  ...
  />
        {showDiscount && product.discount && <span className="absolute top-2 left-2 text-white text-xs font-semibold px-2 py-1 rounded-lg" style={{ backgroundColor: "#e53935" }}>
            -{product.discount}%
          </span>}
        {product.isFlashSale && !product.discount && <span className="absolute top-2 left-2 text-white text-xs font-semibold px-2 py-1 rounded-lg" style={{ backgroundColor: "#f05a22" }}>
            ⚡ Flash
          </span>}
        <button
  ...
  >
      ...
      <div className="p-3 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-1 mb-2">
          ...
          <span className="text-xs text-gray-400 ml-1">Đã bán {product.sold > 1e3 ? `${(product.sold / 1e3).toFixed(1)}k` : product.sold}</span>
        </div>

        <div className="flex items-center gap-2 mb-3">
          <span className="font-semibold text-base" style={{ color: "#1250dc" }}>
  ...
  >
          {added ? "\u2713 \u0110\xE3 th\xEAm" : <span className="flex items-center justify-center gap-1.5">
