...
[TBD: Add 1-2 sentences describing the real-world problem this project solves, such as digitizing pharmacy operations, reducing manual tracking, or supporting online product ordering.]
...
- Admin CRUD workflow for products.
- product category filtering and category count display.
- Product image upload with support for multiple images.
- product search, filtering, and pagination.
- Custom product unit input for flexible product packaging formats.

### Inventory Management

- Inventory quantity tracking for products.
- Stock updates from the admin product form.
...
| `products`       | product and product information       | Many-to-one with `categories` and `manufacturers`   |
| `categories`     | product category groups               | One-to-many with `products`                         |
| `manufacturers`  | product manufacturer information      | One-to-many with `products`                         |
| `product_images` | Additional product images             | Many-to-one with `products`                         |
| `inventory`      | product stock quantity                | One-to-one with `products`                          |
| `inventory_logs` | Inventory movement history            | Many-to-one with `products`                         |
| `orders`         | Customer order records                | One-to-many with `order_items`                      |
| `order_items`    | Order line items                      | Many-to-one with `orders` and `products`            |
| `carts`          | Customer shopping cart                | One-to-many with `cart_items`                       |
| `cart_items`     | Cart line items                       | Many-to-one with `carts` and `products`             |
...
| Method | Endpoint          | Description                                                      | Access |
| ------ | ----------------- | ---------------------------------------------------------------- | ------ |
| GET    | `/products`       | Get product list with search, filtering, sorting, and pagination | Public |
| GET    | `/products/:slug` | Get product detail by slug                                       | Public |
...
| GET    | `/categories/count` | Get categories with product counts | Public |
...
| GET    | `/admin/products`                       | Get all products for admin        | Admin  |
| GET    | `/admin/products/:productId`            | Get product detail for editing    | Admin  |
| POST   | `/admin/products`                       | Create a product                  | Admin  |
| PUT    | `/admin/products/:productId`            | Update product information        | Admin  |
| DELETE | `/admin/products/:productId`            | Soft-delete a product             | Admin  |
...
[TBD: Add notable technical challenges solved, such as Prisma query optimization, upload handling, authentication refresh flow, RBAC route protection, or admin product image management.]
